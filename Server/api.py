...
def rooms():
	...
	data = ""
	for row in rows:
		data += json.dumps({"id" : row[0], "name": row[1]})
	return data
...
def types():
	...
	data = ""
	for row in rows:
		data += json.dumps({"id" : row[0], "name": row[1], "path_image": row[2]})
	return data
...
def room(id):
	...
	data = ""
	for row in rows:
		data += json.dumps({"id" : row[0], "name": row[4]})
	return data
...
def protocoles():
	...
	data = ""
	for row in rows:
		data += json.dumps({"id" : row[0], "name": row[1], "description" : row[2]})
	return data
